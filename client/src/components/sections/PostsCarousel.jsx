  ...
  return (
    <Box as="section" py={{ base: 8, md: 12 }}>
      <HStack justify="center" gap={2} mb={6}>
        <Button variant={tab === "latest" ? "solid" : "ghost"} onClick={() => switchTab("latest")}>
          Latest 10 posts
        </Button>
        <Button variant={tab === "commented" ? "solid" : "ghost"} onClick={() => switchTab("commented")}>
          10 most commented
        </Button>
      </HStack>
      ...
      <HStack justify="center" gap={4} mt={6}>
        <IconButton aria-label="Previous" variant="outline" onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}>
          <LuChevronLeft />
        </IconButton>

        <HStack gap={2}>
          {Array.from({ length: pageCount }).map((_, i) => (
            <Box key={i} w={2.5} h={2.5} borderRadius="full" bg={i === page ? "colorPalette.500" : "gray.300"} />
          ))}
        </HStack>

        <IconButton aria-label="Next" variant="outline" onClick={() => setPage((p) => Math.min(pageCount - 1, p + 1))} disabled={page === pageCount - 1}>
          <LuChevronRight />
        </IconButton>
      </HStack>
    </Box>
